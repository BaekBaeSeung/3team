...
@WebServlet(urlPatterns = {"/dining", "/xxx"},
	initParams = {
...
public class FrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * if(command == null) { System.out.println("file upload event invoked");
		 * 
		 * MultipartRequest mr = MyUtility.getMultipartRequest(request,
		 * imageUploadWebPath);
		 * 
		 * if(mr != null) { // 파일 업로드인 경우에는 mr 객체를 이용해야만 파라미터 정보를 읽어올 수 있습니다. command =
		 * mr.getParameter("command") ;
		 * 
		 * if(command.equals("prUpdate")) { // 상품 정보 수정시 과거 이미지는 삭제해 주어야 합니다.
		 * MyUtility.deleteOldImageFile(imageUploadWebPath, mr); }
		 * 
		 * // file upload object binding in request scope. request.setAttribute("mr",
		 * mr);
		 * 
		 * }else { System.out.println("MultipartRequest object is null"); } }
		 */
		...
		// controller는 해당 command에 상응하는 하위 컨트롤러 객체입니다.
